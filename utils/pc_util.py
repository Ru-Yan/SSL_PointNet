...
# ----------------------------------------
# Point Cloud Sampling     点云采样
# ----------------------------------------

def pc_normal(pointcloud, num_points=20000):  #点云和点数
    ...
    if num_points > raw_num_points: # 如果设定的num_points大于文件中点的数量，那么进行随机化填充
        pt_idxs = np.concatenate([pt_idxs, np.random.randint(0, high=raw_num_points, size=num_points-raw_num_points)])
    else:
        pt_idxs = np.random.randint(0, high=raw_num_points, size=num_points)   #pt_idxs(0,)
...
# ----------------------------------------
# Point Cloud/Volume Conversions   点云体积转化
# ----------------------------------------
