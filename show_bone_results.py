...
def showpoints(xyz,c_gt=None, c_pred = None ,c_pre_logits = None,c_wrong = None,waittime=0,showrot=False,magnifyBlue=0,freezerot=False,background=(255,255,255),normalizecolor=True,ballradius=40):
    global showsz,mousex,mousey,zoom,changed
    xyz=xyz-xyz.mean(axis=0)
    radius=((xyz**2).sum(axis=-1)**0.5).max()
    xyz/=(radius*2.2)/showsz
    if c_gt is None:
        c0=np.zeros((len(xyz),),dtype='float32')+12
        c1=np.zeros((len(xyz),),dtype='float32')+0
        c2=np.zeros((len(xyz),),dtype='float32')+12
    else:
        c0=c_gt[:,0]
        c1=c_gt[:,1]
        c2=c_gt[:,2]
    ...
    while True:
        ...
        if cmd==ord('q'):
            ...
            sys.exit(0)
        #t是原始，p是预测，w是错误
        if cmd==ord('t') or cmd == ord('p') or cmd == ord('w') or cmd == ord('o'):
            if cmd == ord('t'):
                if c_gt is None:
...
if __name__=='__main__':
    ...
    # data = np.loadtxt('Bone_{:04}.txt'.format(int(sys.argv[1])))
    data = np.loadtxt(os.path.join(BASE_DIR,'/home/csc/桌面/SSL-pointnet2/results/pre_bone.txt'))
    if False:
        choice = np.random.choice(len(data), 10000, replace=True)
        vertices = data[choice,:3]
        normal = data[choice,3:6]
        labels = data[choice,-1].astype(np.int32)
    else:
        vertices = data[:,:3]
        normal = data[:,3]
        labels = np.zeros_like(data[:,-1].astype(np.int32))


    showpoints(vertices, c_pred=labels, ballradius=4)
