...
class DataLoaderIter(object):
    ...
    def __init__(self, loader):
        ...
        self.done_event = loader.done_event
        self.batches_outstanding = 0.0

        self.sample_iter = iter(self.batch_sampler)

        if self.num_workers > 0:
            self.worker_init_fn = loader.worker_init_fn
            self.index_queue = loader.index_queue
            self.data_queue = loader.data_queue

            self.send_idx = 0
            self.rcvd_idx = 0
            self.reorder_dict = {}
            self.workers = loader.workers
...
class DataLoader(object):
    ...
    def _init_workers(self):
        ...
        if self.num_workers > 0:
            self.index_queue = _mp_ctx.SimpleQueue()
            self.worker_result_queue = _mp_ctx.SimpleQueue()
            ...
            #  C._update_worker_pids(id(self), tuple(w.pid for w in self.workers))
