...
class PointcloudRotate(object):
    ...
    def __call__(self, points):
        ...
        if not normals:
            ...
            pc_xyz = points[:,:,0:3]
            pc_normals = points[:,:,3:]
            points[:,:,0:3] = torch.matmul(pc_xyz, rotation_matrix.t().cuda())
            points[:,:,3:] = torch.matmul(pc_normals, rotation_matrix.t().cuda())
...
if __name__ == '__main__':
    a = PointcloudScale()
    b = a(torch.rand(3,20000,6).cuda())
    c = torch.rand(3,20000,6).cuda().size(2)
    print(b)